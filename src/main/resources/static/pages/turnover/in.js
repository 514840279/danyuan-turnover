window.operateEvents = {
	...
	'click #clickA ': function (e, value, row, index) {
		$("#income_uuid").val(row.uuid);
		$("#income_deleteFlag").val(row.deleteFlag);
		$("#income_discription").val(row.discription);
		$("#income_name").val(row.name);
		$("#income_money").val(row.money);
		$("#income_type").val(row.type).trigger('change');
...
$(function() {
	...
	$("#addnew_income").click(function(){
		$("#income_uuid").val("");
		$("#income_deleteFlag").val(0);
		$("#income_discription").val("");
		$("#income_name").val("");
		$("#income_money").val("");
		$("#income_type").val("类型").trigger('change');
	...
	$("#income_save_button").click(function(){
		var url="/sysTurnoverInfo/save";
		var param={
			uuid:	$("#income_uuid").val(),
			deleteFlag:$("#income_deleteFlag").val(),
			discription:$("#income_discription").val(),
			name:$("#income_name").val(),
			money:$("#income_money").val(),
			type:$("#income_type").val(),
...
function init(){
	...
	$("#income-table-id").bootstrapTable({
		url : "/sysTurnoverInfo/pageIncome",
		dataType : "json",
		toolbar : '#income_toolbar', // 工具按钮用哪个容器
